...
> ⚠️ **Revisión del 2026-07-31 — la llamada multi-sede se retira.**
>
> Este plan se escribió el 2026-07-30 con **llamada multi-sede simultánea**, tal y como la especificaba la versión de esa fecha del diseño: varias sedes destino, todas sonando a la vez, estado en vivo por sede en la pantalla `llamando` e incorporación de una tercera sede a una llamada en curso.
>
> El 2026-07-31 negocio retiró ese requisito: **nunca más de dos sedes en una llamada**. Las tareas 2, 4, 5, 6, 7, 9, 10 y 11 de este plan se han actualizado en consecuencia, y el multi-sede se ha eliminado del código por completo en vez de dejarlo desactivado.
>
> **Elegir a qué sede se llama no cambia:** con tres oficinas, Lorca sigue escogiendo entre Murcia y Canarias. Lo retirado es solo *hacer sonar varias a la vez*.
>
> Si en el historial de git aparecen `destinos: string[]`, `aceptadas`, `estadosDestino`, `publicar-invitaciones`, `Invitacion.invitados` o el evento `se-une`, **no hay que restaurarlos**: son una decisión de negocio revertida a propósito. Ver la nota equivalente en el diseño.
...
describe('maquina de estados: llamada saliente', () => {
    ...
    it('confirmar la seleccion publica UNA invitacion al unico destino', () => {
        ...
        expect(invitaciones[0]).toMatchObject({ destino: 'murcia' });
    });
    ...
    it('una aceptacion de una sede que NO es el destino no entra en la llamada', () => {
        // Con llamadas 1 a 1 esto seria colar a un tercero.
        const sel = transicion(inactivo(), { tipo: 'toque-pantalla' }).contexto;
        const llamando = transicion(sel, { tipo: 'seleccion-confirmada', destino: 'murcia' }).contexto;
    ...
    it('durante la llamada, una aceptacion ajena no vuelve a crear el iframe', () => {
        const sel = transicion(inactivo(), { tipo: 'toque-pantalla' }).contexto;
        const llamando = transicion(sel, { tipo: 'seleccion-confirmada', destino: 'murcia' }).contexto;
...
export function transicion(ctx: Contexto, evento: Evento): Resultado {
    ...
    switch (ctx.estado) {
        ...
        case 'en-llamada': {
            // Con llamadas 1 a 1 no hay incorporaciones: el unico invitado ya ha
            // contestado, asi que un `sede-acepto` aqui solo puede ser una
            // reentrega del broker o una sede ajena. En ambos casos, nada.
            return sinCambios(ctx);
        }
...
Ejecuta: `npx vitest run src/core/maquina-estados.test.ts`
Esperado: 17 tests en verde (7 de la tarea 3 + 10 nuevos).
...
> ⚠️ **Corrección detectada durante la ejecución (2026-07-30).** El guard de `broker-desconectado` que introduce la tarea 3 y conserva la tarea 4 dispara desde cualquier estado, incluido `en-llamada`, y sale sin emitir `destruir-jitsi`: el iframe queda huérfano. Es un defecto de este plan contra el propio spec.
>
> El spec §3.2 exige que una caída del broker **no corte la llamada** — señalización y media están desacoplados, el vídeo va por Jitsi. Por tanto la máquina no debe abandonar `en-llamada` ni `finalizando` ante `broker-desconectado`; debe caer a `sinCambios`. Con eso el `colgar` posterior emite su `destruir-jitsi` y la fuga desaparece.
>
> Esta tarea incorpora el arreglo y cuatro tests de regresión. Si reejecutas el plan desde cero, aplica la excepción del guard ya en la tarea 3.
...
export class ClienteMqtt {
    ...
    async conectar(): Promise<void> {
        ...
        this.cliente = mqtt.connect(this.op.url, {
            ...
            will: {
                // String, no Buffer: will.payload acepta ambos y Buffer no existe
                // en el navegador. Envolverlo obligaria a shimear un global de Node
                // en el bundle de tres kioscos, para nada.
        ...
        // Aqui NO se publica la presencia. La disponibilidad la decide la maquina
        // de estados, que es la unica que sabe si hay una llamada en curso: esta
        // capa publicando `('libre', null)` en cada CONNACK dejaba el retenido
        // mintiendo el resto de la llamada tras cualquier parpadeo del broker.
        // Es `broker-conectado` quien emite el `publicar-estado` correcto.
        this.cbConectar();
    }
    ...
    async desconectar(): Promise<void> {
        ...
        // Publica online:false, NO publicarEstado(): un endAsync() envia un DISCONNECT
        // limpio y el broker descarta el Will, asi que el cliente tiene que retirar
        // su propia presencia o el retained se queda diciendo online:true para siempre.
...
export class SesionJitsi {
    ...
    crear(sala: string): void {
        ...
        this.api = nuevaApi;
        // La comprobacion de identidad no sobra: una api ya destruida puede emitir
        // videoConferenceLeft con retraso, y sin ella ese evento tardio senalaria
        // un fallo en la sesion ACTUAL, tumbando una llamada que esta sana.
...
export class Interprete {
    ...
    private async uno(efecto: Efecto): Promise<void> {
        switch (efecto.tipo) {
            ...
            // de salida de `finalizando` y este switch corre dentro de un
            // try/catch que aisla cada efecto. Emitiendolo solo en la via de
            // exito, un `dispose()` que lanzara dejaba la maquina atrapada con la
            // pantalla congelada hasta recargar.
            case 'destruir-jitsi':
                try {
                    this.jitsi.destruir();
                } finally {
...
- Produce: `function render(raiz: HTMLElement, vista: Vista): void` con
  `interface Vista { contexto: Contexto; sedes: EstadoSede[]; seleccion: string | null; reloj: string }`
...
Reejecuta `npm test` y confirma que las 29 pruebas de la FSM y las 5 de MQTT
...
describe('pantallas', () => {
    ...
    it('marca las sedes OCUPADAS como no seleccionables, igual que las offline', () => {
        ...
        expect(raiz.querySelector<HTMLButtonElement>('[data-sede="canarias"]')!.disabled).toBe(true);
        expect(raiz.querySelector<HTMLButtonElement>('[data-sede="murcia"]')!.disabled).toBe(false);
    });
...
// El nombre de sede y el origen llegan por MQTT desde otros totems y desde el
// topic retenido config/sedes: no son constantes de codigo y no se pueden
// interpolar crudos en innerHTML. Un totem con credenciales validas podria
// inyectar markup en las demas pantallas, que son kioscos desatendidos con
// permiso de camara y microfono ya concedido.
...
function tarjetas(v: Vista, seleccionable: boolean): string {
    // Una sede OCUPADA no es pulsable, igual que una offline: con llamadas 1 a 1
    // nadie se incorpora a una conversacion en curso, asi que llamar a quien ya
    // esta hablando solo produciria 45 s de timbre que nadie va a contestar.
    const libre = (s: EstadoSede) => s.online && s.disponibilidad !== 'ocupado';
    return v.sedes.map(s => `
        <button class="${claseSede(s)}" data-sede="${s.sede}"
                ${seleccionable && libre(s) ? '' : 'disabled'}
...
<head>
    ...
    <!-- Autoalojado a proposito. NO apuntar a meet.sunube.net: el arranque del
         totem no puede depender de que responda el dominio del tercero. Caddy
         sirve este fichero desde /vendor (tarea 12). -->
    <script src="/vendor/external_api.js"></script>
</head>
<body>
    <main id="app"></main>
...
raiz.addEventListener('click', ev => {
    ...
    if (sede !== undefined && contexto.estado === 'seleccionando') {
        // 1 a 1: elegir otra sede SUSTITUYE a la anterior, no la anade.
        seleccion = seleccion === sede ? null : sede;
        pintar();
        return;
    }
    ...
    switch (objetivo.dataset.accion) {
        ...
        case 'llamar': {
            if (seleccion !== null) emitir({ tipo: 'seleccion-confirmada', destino: seleccion });
            break;
        }
